...
class EdgeConnect():
    def __init__(self, config):
        ...
        self.inpaint_model = InpaintingModel(config).to(config.DEVICE) # 图像补全模型
 
        self.psnr = PSNR(255.0).to(config.DEVICE) # 计算信噪比
        self.edgeacc = EdgeAccuracy(config.EDGE_THRESHOLD).to(config.DEVICE) # 计算准确度
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for items in train_loader:
                self.edge_model.train() # 训练边缘生成器
                self.inpaint_model.train() # 训练图像补全模型
