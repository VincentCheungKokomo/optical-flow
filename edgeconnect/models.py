...
from networks import InpaintGenerator, Discriminator, EdgeGenerator_
...
class EdgeModel(BaseModel): 
    ...
    def __init__(self, config):
        ...
        generator = EdgeGenerator(use_spectral_norm=True) # 生成器
        discriminator = Discriminator(in_channels=2, use_sigmoid=config.GAN_LOSS != 'hinge') # 判别器
        if len(config.GPU) > 1: # 若GPU > 2 
            generator = nn.DataParallel(generator, config.GPU)
            discriminator = nn.DataParallel(discriminator, config.GPU)
        l1_loss = nn.L1Loss() # l1 损失
