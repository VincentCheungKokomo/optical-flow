...
def evaluate(args):   
    ...
    print('grountruth_folder_list length: ',len(grountruth_folder_list))
    print('prediction_folder_list length: ',len(prediction_folder_list))

    for i in range(0,len(grountruth_folder_list)):
        # load video
        video_gt=[]
        video_predict=[]
        image_list = sorted(glob.glob(os.path.join(grountruth_folder_list[i],'*.png')))
        ...
        for image_gt in image_list:
            ...
            try: 
                image_predict=prediction_folder_list[i]+image_gt[-10:]
        ...
        for j in range(0,len(image_list)):
            print(video_gt[j].shape, video_predict[j].shape)
            if video_gt[j].shape!=video_predict[j].shape:
                error_code=-3
                error_flag= 'image size unmatch. please check video_' + str(i).zfill(4)+str(j).zfill(4)
                return error_code, error_flag, 0
