...
# 在f2中搜索f1，
    ...
    # print("D shape: ", D.shape)  # 7680, 8
    # print("I shape: ", I.shape)  # 7680, 8
    return D, I


def knn_faiss_raw(fmap1, fmap2, k):
    # print('***********fmap1 shape : ', fmap1.shape) 
    b, ch, _ = fmap1.shape # 1, 256, 7680


    if b == 1:
        fmap1 = fmap1.view(ch, -1).t().contiguous() # 7680, 256
        fmap2 = fmap2.view(ch, -1).t().contiguous() # 7680, 256
        # 在fmap2中搜索fmap1，不构建index， 衡量指标为内积
        # print(fmap1.shape, fmap2.shape, k)  #  k = 8
        dist, indx = search_raw_array_pytorch(res, fmap2, fmap1, k, metric=faiss.METRIC_INNER_PRODUCT) # 返回距离和索引
        # print('dist: ', dist.shape)  # 7680, 8
        # print('index: ', indx.shape) # 7680, 8
        dist = dist.t().unsqueeze(0).contiguous()  # 1, 8, 7680
        indx = indx.t().unsqueeze(0).contiguous()  # 1, 8, 7680
