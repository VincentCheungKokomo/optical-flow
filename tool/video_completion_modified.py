...
def find_minbbox(masks):
    # find the minimum bounding box of the holdmask  找到仍然存在的缺失区域
    minbbox_tl=[]            # 左上 top left point of the minimum bounding box
    minbbox_br=[]            # 右下 bottom right point of the minimum bounding box
    for i in range (0,len(masks)): # 对每一帧图像进行处理
...
def video_completion_seamless(args):
    ...
    # Obtains imgH, imgW and nFrame. 获取视频图像的长宽以及帧数
...
if __name__ == '__main__':
    ...
    # RAFT
    parser.add_argument('--model', default='./weight/raft-things.pth', help="restore checkpoint") # --model 获取RAFT模型的权重
